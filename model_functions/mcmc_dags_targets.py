...
def mcmc_dag_targets(X, S, burn, w, a=None, a_k=None, b_k=None, n_all=None):
    ...
    D = [np.zeros((n_all[k], q)) for k in range(K)]
    ...
    out_D = [np.zeros((n_all[k], q)) for k in range(K)]
    ...
    for s in range(1, S + 1):
        ...
        if (s % 5 == 0) and (s > burn):
            ...
            for k in np.random.choice(K, K, replace=False):
                ...
                logprior_k = (loggamma(a_k + np.sum(I_cal_prop[:, k])) + loggamma(q - np.sum(I_cal_prop[:, k]) + b_k) -
                              loggamma(a_k + np.sum(I_cal[:, k])) - loggamma(q - np.sum(I_cal[:, k]) + b_k))
